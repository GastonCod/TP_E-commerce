class ToastIt {
  ...
  static pause = !1;
  static closeButton = !1;
  static now({ style: e, message: t, timer: s, close: a, position: o }) {
    switch (
      ((this.message = t || this.getMessageFromLang()),
      (this.closeButton = a),
      (this.timer = s > 1e4 ? 3500 : s),
      e.toLowerCase() || "")
    ) {
      case "info":
    ...
    i.classList.add("toast-div"),
      i.classList.add("toast-generic"),
      i.classList.replace("toast-generic", this.style),
      (i.innerHTML = `${this.message}`),
      (i.style.zIndex = "999999"),
      (i.style.opacity = 0);
    const r = document.querySelectorAll(".toast-div").length;
    let n;
    switch (o?.trim().toLowerCase()) {
      case "center":
        ...
        break;
      case "left":
        (n = "20px"), (i.style.left = "20px");
        break;
      default:
        (n = 20 + 60 * r + "px"), (i.style.right = "20px");
    }
    if (((i.style.top = n), this.closeButton)) {
      const e = document.createElement("span");
      e.classList.add("toast-span-close"),
        (e.style.fontSize = "larger !important"),
        (e.textContent = " ⓧ"),
    ...
    document.querySelector("dialog[open]")
      ...
      : document.querySelector("body").insertAdjacentElement("beforebegin", i),
      (i.style.textAlign = "justify !important");
    let l = 0;
    const c = setInterval(() => {
      l++,
        (i.style.opacity = (l / 130).toFixed(1)),
        l >= 130 && clearInterval(c);
    }, 0.1);
    this.pause ||
      setTimeout(() => {
        let e = 150;
        const t = setInterval(() => {
          e--,
            (i.style.opacity = (e / 150).toFixed(1)),
            e <= 0 && clearInterval(t);
        }, 0.1);
